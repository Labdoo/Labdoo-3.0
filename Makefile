...
migrate-revisions: ## 🔄 Migrate node revisions (e.g., make migrate-revisions type=page [limit=10] [body-field=field_body] [nids=1,2,3] [dry-run=1] [incremental=1] [destination_type=dootronic] [delete-revisions=1]).
	@if [ -z "$(type)" ]; then \
	...
	$(DRUSH_COMMAND) labdoo-sync-revisions $(type) $(if $(limit),--limit=$(limit),) $(if $(body-field),--body-field=$(body-field),) $(if $(nids),--nids=$(nids),) $(if $(dry-run),--dry-run,) $(if $(incremental),--incremental,) $(if $(destination_type),--destination-type=$(destination_type),) $(if $(delete-revisions),--delete-revisions,)
...
.PHONY: fix-node-uids
fix-node-uids: ## 👤 Fix nodes with uid=0 checking original author in D7. Usage: make fix-node-uids [type=TYPE] [limit=N]
	@echo "$(CYAN)👤 Fixing nodes with uid=0...$(RESET)"
	$(DRUSH_COMMAND) labdoo:fix-node-uids $(if $(type),--type=$(type),) $(if $(limit),--limit=$(limit),)
