...
class SourceRepository implements SourceRepositoryInterface {
   ...
   *   The external connection manager.
   * @param \Drupal\labdoo_migrate\Services\SourceContent\TranslationRepositoryInterface $translationRepository
   *   The translation repository.
   ...
   * @param bool $ignoreNid
   *   If TRUE, ignores the nid field.
   *
   * @return array
  ...
  protected function getFieldValues(
    ...
    foreach ($this->mapping as $mapping) {
      if ($ignoreNid && $mapping->getSourceIdentifier() === 'node.nid') {
        $entity[$mapping->getSourceIdentifier()] = NULL;
  ...
  protected function getSingleMappingValue(
    ...
    if ($field->getDefaultValue()) {
      return $field->getDefaultValue()[$langCode];
    }
  ...
  protected function getQueryResults(
    ...
    if (!$field->ignoreLangcode() && $langCode !== NULL) {
      $query->condition('language', [$langCode, 'und'], 'IN');
    }

    if ($specialCondition !== NULL) {
      $query->where($specialCondition);
    }
  ...
  protected function processSingleResult(
    ...
    $originalTranslation = $this->translationRepository
      ->getOriginalTranslation($entityId, $field, $mainLangCode);
