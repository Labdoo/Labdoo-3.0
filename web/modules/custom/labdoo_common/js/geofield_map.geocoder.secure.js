(function ($, Drupal, drupalSettings) {
  ...
  Drupal.geoFieldMapGeocoder = {
    ...
    get_path_prefix: function () {
      let rawPrefix = (drupalSettings.path && drupalSettings.path.pathPrefix) || '';
      return rawPrefix.replace(/^\/+|\/+$/g, '');
    },

    build_endpoint: function (endpoint) {
      let prefix = this.get_path_prefix();
      return prefix ? '/' + prefix + '/geocoder/api/' + endpoint : '/geocoder/api/' + endpoint;
    },

    normalize_providers: function (providers) {
      if (Array.isArray(providers)) {
        providers = providers.join(',');
      }
      providers = (providers || '').toString().trim();
      return providers;
    },

    geocode: function (address, providers, options) {
      let self = this;
      let geocode_path = self.build_endpoint('geocode');
      let normalizedProviders = self.normalize_providers(providers);
      options = self.query_url_serialize(options);
      return $.ajax({
        url: geocode_path + '?address=' + encodeURIComponent(address) + (normalizedProviders ? '&geocoder=' + encodeURIComponent(normalizedProviders) : '') + '&' + options,
        type:"GET",
    ...
    reverse_geocode: function (latlng, providers, options) {
      let self = this;
      let reverse_geocode_path = self.build_endpoint('reverse_geocode');
      let normalizedProviders = self.normalize_providers(providers);
      options = self.query_url_serialize(options);
      return $.ajax({
        url: reverse_geocode_path + '?latlng=' + latlng + (normalizedProviders ? '&geocoder=' + encodeURIComponent(normalizedProviders) : '') + '&' + options,
        type:"GET",
        contentType:"application/json; charset=utf-8",
        dataType: "json",
      });
    ...
    map_control_autocomplete: function (mapid, geocoder_settings, selector, type, map_library) {
      let self = this;
      let providers = self.normalize_providers(geocoder_settings.providers);
